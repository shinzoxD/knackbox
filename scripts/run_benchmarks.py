...
def normalize_choice(value: str, known_names: set[str]) -> str:
    normalized = value.strip().lower().splitlines()[0] if value.strip() else "none"
    normalized = normalized.strip(" .`'\"")
    return normalized if normalized in known_names else "none"
...
def majority_target(choices: list[str], target: str) -> bool:
    ...
    if not counts:
        return False
    return counts.most_common(1)[0][0] == target
...
def normalize_judgment(value: str) -> str:
    normalized = value.strip().lower().splitlines()[0] if value.strip() else "tie"
    normalized = normalized.strip(" .`'\"")
    return normalized if normalized in {"first", "second", "tie"} else "tie"
...
def main() -> int:
    ...
    suites = {skill["name"]: load_json(benchmark_path(skill)) for skill in skills}
